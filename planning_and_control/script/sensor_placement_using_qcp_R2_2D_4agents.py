...
class sensor_placement_optimization(object):
    def __init__(self): 
        ...
        self.no_of_sensors = 4
        self.dimension = 2 # dimension: 2D or 3D
        self.drr = 1.25 # minimum distance between robots
        ...
        self.robots = [self.x1, self.x2, self.x3, self.x4]
    ...
    def plot(self, xxt, x1, x2, x3, x4): 
        ...
        plt.plot(xx4, yy4, marker = 'o', color='k', markersize = 1, linestyle='--', label='Mav4')
        
        #print xx1, yy1
        for i in range(len(xx1)): 
            if i == 0: 
                x = [xx1[i], xx2[i], xx3[i], xx4[i], xx1[i], xx3[i], xx2[i], xx4[i]]
                y = [yy1[i], yy2[i], yy3[i], yy4[i], yy1[i], yy3[i], yy2[i], yy4[i]]
    ...
    def optimize(self):
        ...
        for d in self.time_points: 
            ...
            px = m.addVars(n, vtype=GRB.CONTINUOUS, name="px")
            py = m.addVars(n, vtype=GRB.CONTINUOUS, name="py")
            """
            ...
            w = [0.9, 0.1, 0.1, 0.1]; w = w/np.linalg.norm(w)
            #for q in range(n): 
            #    m.addConstr((x[q]-self.xt[0]) == px[q])
            #    m.addConstr((y[q]-self.xt[1]) == py[q])
            #obj1 = quicksum(w[i]*px[i] * quicksum(Q[i][j] * px[j] for j in range(n)) for i in range(n))
            #obj2 = quicksum(w[i]*py[i] * quicksum(Q[i][j] * py[j] for j in range(n)) for i in range(n))
