...
class squircleworld_NF(object):
    def __init__(self, name_of_uav, number, velocity, initial_hovering_height, freq): 
        ...
        self.O= [\
                 [0.0, 0.0, 7.5, 15.0, 0.9999, 0.0, 1.0, 1.0, 0.5], \
        ...
        open("navigation_function_for_firefly{}.txt".format(self.no), "w").close()
        open("goal_for_firefly{}.txt".format(self.no), "w").close()
        open("smoothing_factor_for_firefly{}.txt".format(self.no), "w").close()
        ...
        self.pub1 = rospy.Publisher('/reached_goal', Point, queue_size = 1, tcp_nodelay = True)
        self.pub = rospy.Publisher('/firefly'+self.no+'/NavigationFunctionTrajectory', velocities_from_navigation_function, queue_size = 1, tcp_nodelay = True)
        try:
            rospy.Subscriber('/target_positions_for_quads', target_positions, self.callback1, queue_size = 1, tcp_nodelay = True)
            rospy.Subscriber('/firefly'+self.no+'/odometry_sensor1/odometry', Odometry, self.callback2, queue_size = 1, tcp_nodelay = True)
        except:
    ...
    def callback1(self, tp): 
        ...
        f1 = open('goal_for_firefly{}.txt'.format(self.no), 'a')
    ...
    def distance_to_goal(self, q): 
        ...
        return nrm(q - self.goal)**2.0#2.2
    ...
    def generate_velocity_trajectory(self, phi_, X_, odom): 
        ...
        distancefactor = scipy.special.erf(5.0*(dist_to_goal-self.proximity))
    ...
    def callback2(self, odom): 
        ...
        if self.planning_counter  == 0: 
        ...
        elif self.planning_counter > 0 and self.planning_counter < 200: 
            ...
            phi = self.navigation_function(h)
            f = open('navigation_function_for_firefly{}.txt'.format(self.no), 'a')
            f.write("%s, %s\n" % (time.time(), phi[2]))
...
if __name__ == '__main__':
    ...
    traj = squircleworld_NF(name, number, velocity, initial_hovering_height, freq)
    try:
