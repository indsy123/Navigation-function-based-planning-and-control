...
class velocity_controller(object): 
    def __init__(self, name_of_uav,parameters): 
        ...
        self.dt = 1.0/100 # time step based on 150Hz frequency
        self.ei = 0
	...
	#self.ki_att = np.array([[3], [3], [3]])
	    ...
	    #odom = message_filters.Subscriber('/'+self.uav+'/odom', Odometry, tcp_nodelay = True)
            traj = message_filters.Subscriber('/'+self.uav+'/NavigationFunctionTrajectory', velocities_from_navigation_function, tcp_nodelay = True)
	...
	#    self.ei = ei 
        ...
        ff = open('b2c.txt', 'a')
        ff.write("%s, %s, %s, %s\n" % (b2c[0][0], b2c[1][0], b2c[2][0], np.linalg.norm(np.cross(b3c.T, b1d.T))))
